 ...
 * ★ reviewId 는 unified_review 에 public_id 컬럼이 없어(docs/11) BIGSERIAL id 를 문자열로 노출한다
 * (DraftDtos 와 동일한 사유·동일한 임시조치).
 */
final class ReviewDtos {
    ...
    record ReviewListResponse(List<ReviewSummaryResponse> items, boolean hasMore) {
    }
