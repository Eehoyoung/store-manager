...
class AuthServiceTest {
    ...
    void 회원가입후_로그인_해피패스() {
        ...
        AuthService.TokenPair signupResult = authService.signup(signupReq, "127.0.0.1", "test");
    ...
    void 일반사용자는_가맹코드없이_브랜드없는_매장으로_가입한다() {
        ...
        AuthService.TokenPair result = authService.signup(req, null, null);
    ...
    void 필수동의가_빠지면_사용자와_매장을_만들지_않는다() {
        ...
        verifyNoInteractions(storeService, franchiseService);
    }
    ...
    void 가맹코드가_있어도_본부제공_미동의면_가입만_하고_거부를_기록한다() {
        ...
        when(passwordEncoder.encode(req.password())).thenReturn("hash");
        when(appUserRepository.save(any())).thenAnswer(inv -> inv.getArgument(0));
        Store store = Store.builder().id(2L).ownerId(1L).name("매장").build();
        when(storeService.createStore(any(), anyString(), anyString())).thenReturn(store);
        stubTokenIssuance();

        AuthService.TokenPair result = authService.signup(req, "127.0.0.1", "test");

        assertThat(result.affiliationRequested()).isFalse();
        verify(franchiseService, never()).requestAffiliation(any(), any(), anyString());
        verify(agreementService).record(result.user().getId(), store.getId(), AgreementService.HQ, false,
                "127.0.0.1", "test");
    }
