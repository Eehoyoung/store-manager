...
PROMPT_VERSION = "v1.8"  # risk 1/2 판정 순서 명시 · ABUSIVE 정의 이원화 (v1.7 실측 반영)
...
사용 가능한 값: {", ".join(RISK_REASON_VALUES)}
...
· ABUSIVE     아래 [ABUSIVE 와 COMPLAINT 의 경계] 참조.
· NOISE       본문이 비었거나 이모지·자모음·말줄임표뿐이다. 예) "😊"  "ㅎㅎ"  "..."
              ★ 내용이 있는데 매장과 무관하면 NOISE 가 아니라 ABUSIVE(2) 다.
...
[issue_tags]
리뷰가 **실제로 언급한 것만** 고른다. 없으면 빈 배열이다. 보통 0~3개이며 억지로 채우지 않는다.
...
- ABUSIVE 와 COMPLAINT 의 경계 (가장 자주 틀리는 지점이다)
  · ABUSIVE: 둘 중 하나면 ABUSIVE 다.
    ...
    (2) 리뷰가 아닌 것 — 광고·홍보·영업 문의, 매장과 무관한 신변잡담·시사 논평.
        공격이 없어도 여기에 해당하면 ABUSIVE 다. 자동 답글을 달 대상이 아니다.
        예) "여기 광고 문의 주세요 마케팅 대행합니다"
            "요즘 세상 돌아가는 꼴이 다 이렇지 뭐 한심하다"
        ★ 내용은 있는데 이 매장 이야기가 아니면 NOISE 가 아니라 여기다.
          NOISE 는 **내용 자체가 없을 때**만 쓴다.
  · COMPLAINT: 음식·배달·응대의 문제를 지적하되 공격 표현이 없으면 COMPLAINT 다.
    "너무 짜요", "40분이나 늦었어요", "다시는 안 시킬 것 같아요" 는 COMPLAINT.
  · 판단 기준은 표현의 세기가 아니다. (1)은 **공격 대상이 사람·매장 자체인가**,
    (2)는 **이 글이 이 매장의 음식·배달·응대에 대한 것인가** 로 가른다.
...
2. 이 글이 이 매장의 음식·배달·응대 이야기가 맞는가? 아니면 ABUSIVE(2) 다.
...
_THIRD_PARTY = re.compile(r"다른\s*(손님|사람|고객|분|주문)|타인|남의|옆집|배달원|라이더|기사")
...
def _hits(body: str, reason: str) -> bool:
    ...
    if reason == "PRIVACY_LEAK":
        ...
        if not _THIRD_PARTY.search(body):
            return False
        return _PHONE_PATTERN.search(body) is not None or _PII_ITEM.search(body) is not None
    return False
...
SITUATION_GUIDE: dict[str, str] = {
    ...
    "맛": "맛이 기대에 못 미친 상황이다. 입맛은 사람마다 다르므로 반박하지 마라. "
          "기대에 못 미친 점을 인정하고, 레시피·조리를 다시 보겠다고 한 가지만 구체적으로 말하라.",
    ...
    "양": "양이 적다는 지적이다. 반박하지 말고 계량을 다시 확인하겠다고 말하라. "
          "★ 양을 늘려 드리겠다는 약속은 하지 마라 — 지키지 못하면 더 큰 불만이 된다.",
    "온도": "음식이 식어서 도착한 상황이다. 포장 보온과 조리 후 출고 간격을 짚어라. "
            "배달 소요는 매장이 통제하지 못하는 부분이 있으니 원인을 단정하지 마라.",
    "조리상태": "조리 상태 문제다(면이 불었다·덜 익었다·탔다). 조리 시점과 출고 간격 "
                "문제일 수 있다. 그 사실을 그대로 짚어 사과하고 조리·출고 타이밍을 "
                "다시 보겠다고 구체적으로 말하라.",
    "신선도": "재료 신선도 지적이다. 발주·보관 주기를 다시 보겠다고 말하라. "
              "★ 상했다·냄새가 난다 수준이면 사람이 검수하는 건이라 여기까지 오지 않는다.",
    "용기": "용기 문제다(파손·규격). 어떤 용기로 바꾸겠다는 것인지 한 가지만 말하라.",
    "청결": "청결 지적이다. 매장·조리도구 점검을 다시 하겠다고 말하라. "
            "★ 이물질·곰팡이 수준이면 사람이 검수하는 건이라 여기까지 오지 않는다.",
    # ── 매장 밖 요인 ────────────────────────────────────────────────────────
