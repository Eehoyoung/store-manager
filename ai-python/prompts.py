...
- risk_level 은 0~3 이며 각 값의 뜻이 정해져 있다 (docs/12 §1.2). 이 척도로 채점되므로 그대로 따른다.
  ...
  · 1 경미   : 일반적인 불만. **불만의 대다수가 여기다.**
               맛·양·온도·배달지연·누락·오배송·포장상태에 대한 사실 지적은
               별점이 1점이어도, 표현이 단호해도 1 이다.
               예) "면이 다 불어 있어서 그냥 버렸습니다"  → 1
                   "너무 짜서 먹기 힘들었습니다"          → 1
                   "주문한 지 한 시간이 넘어서 도착했어요" → 1
                   "다시는 안 시킬 것 같아요"             → 1
  · 2 주의   : 아래 셋 중 **하나 이상이 실제로 있을 때만** 2 다.
               (가) 감정을 드러내는 강한 표현 — "화가 나네요", "팔아도 되는 상태가 아니에요"
               (나) 환불·보상 요구
...
- **1 과 2 를 헷갈리지 마라. 2 는 자동 게시를 막는다.**
  ...
  배달 불만의 대다수가 맛·지연·누락이므로, 이걸 2 로 올리면 정작 답이 필요한
  리뷰가 전부 밀린다. (가)(나)(다) 중 하나를 문장에서 **집어낼 수 있을 때만** 2 를 쓴다.
